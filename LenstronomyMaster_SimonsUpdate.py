...
def getPSF(data_class, deltaPix, kwargs_data, x_quasar, y_quasar, lens_cropped, kernel_size, Path, LensName, PSF_Choice, PSF_Int_Kernel, PSF_Rot):
    ...
    # kernel_point_source_new = kernel_util.cutout_source(x_quasar[0], y_quasar[0], kwargs_data['image_data'],
    ...
    psf_error_map = np.ones_like(kernel_point_source_new) * PSF_Int_Kernel
...
def setParameters(theta_e_est, ra_lens, dec_lens, ra_quasar, dec_quasar, lens_params_dict, source_params_dict, lens_light_params_dict, ps_params_dict):
    ...
    kwargs_lens_init = [
        ...
        {'e1': 0., 'e2': 0.}]
    kwargs_source_init = [
        {'R_sersic': 0.02, 'n_sersic': 2., 'e1': 0.2, 'e2': 0, 'center_x': ra_lens, 'center_y': dec_lens}]
    kwargs_lens_light_init = [
        {'R_sersic': 0.5, 'n_sersic': 2., 'e1': 0., 'e2': 0., 'center_x': ra_lens, 'center_y': dec_lens}]
...
def lenstronomy_master(LensName, file_path, exp_time, bkg_rms, bkg_mean, x_quasar, y_quasar, crop_factor, lens_org, lens_cropped, kernel_size, lens_params_dict, source_params_dict, lens_light_params_dict, ps_params_dict, n_iterations, n_particles, PSF_Choice, PSF_Int_Kernel, SourceVisible, PSF_Rot, *mask_path):
   ...
   kwargs_psf, ra_quasar, dec_quasar, ra_lens, dec_lens, theta_e_est = getPSF(data_class, deltaPix, kwargs_data, x_quasar, y_quasar, lens_cropped, kernel_size, Path, LensName, PSF_Choice, PSF_Int_Kernel, PSF_Rot)
   ...
   fitting_kwargs_list = [
       ...
       {'fitting_routine': 'psf_iteration', 'psf_iter_num': psf_iterations, 'psf_iter_factor': 0.2},
...
for i in range(len(Names)):
    if RunBool[i] == 1:
        ...
        lenstronomy_master(Names[i], Paths[i], ExpTime[i], BKG_RMS[i], BKG_MEAN[i], [Quasar1_x[i], Quasar2_x[i]],
                           ...
                           ast.literal_eval(lens_light_params_dict[i]), ast.literal_eval(ps_params_dict[i]), n_iterations[i], n_particles[i], PSF_Choice[i], PSF_Int_Kernel[i], SourceVisible[i], PSF_Rot[i], Mask[i])
    else:
